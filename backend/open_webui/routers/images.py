    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-CryoTensor-User-Name"] = quote(user.name, safe=" ")
                headers["X-CryoTensor-User-Id"] = user.id
                headers["X-CryoTensor-User-Email"] = user.email
                headers["X-CryoTensor-User-Role"] = user.role

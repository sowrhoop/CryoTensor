...
import { verifyToolServerConnection } from '$lib/apis/configs';
	...
	const init = () => {
		if (connection) {
			...
			if (!['none', 'bearer', 'session'].includes(auth_type)) {
				auth_type = 'bearer';
			}
			key = connection?.key ?? '';
				...
				>
					...
					{#if type === 'mcp'}
						...
						>
							...
							{$i18n.t(
								'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the CryoTensor team, making it the more reliable option for compatibility.'
							)}
